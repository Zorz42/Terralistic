...
pub fn compile_resource_pack(input_resource_pack: PathBuf, output_resource_pack: PathBuf) {
    ...
    println!("cargo:rerun-if-changed={}", input_resource_pack.to_str().unwrap());
    ...
    for entry in std::fs::read_dir(input_resource_pack).unwrap() {
        ...
        // if its a folder then recursively call this function
            ...
            compile_resource_pack(path.clone(), output_resource_pack.join(path.file_name().unwrap()));
        }
        ...
        // else just copy the file
            ...
            std::fs::copy(path.clone(), output_resource_pack.join(path.file_name().unwrap())).unwrap();
        }
    }
}