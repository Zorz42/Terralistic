...
impl Background {
    ...
    pub fn render(&self, graphics: &mut GraphicsContext, camera: &Camera) {
        ...
        let position_x = -((camera.get_position().0 * scale / 20.0) as i32) % ((self.image.get_texture_width() as f32 * scale) as i32);
        //for(int i = 0; i < gfx::getWindowWidth() / (background.getTextureWidth() * scale) + 2; i++)
        for i in 0..(graphics.renderer.get_window_width() as f32 / (self.image.get_texture_width() as f32 * scale) + 2.0) as i32 {
            //background.render(position_x + i * int(background.getTextureWidth() * scale), 0, scale);
            self.image.render(&graphics.renderer, scale, position_x + i * (self.image.get_texture_width() as f32 * scale) as i32, 0, None, false, None);
        }
    }
}