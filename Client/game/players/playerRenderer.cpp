...
void ClientPlayers::render(OtherPlayer& player_to_draw) {
    ...
    player_to_draw.name_text.render(1, gfx::getWindowWidth() / 2 - player_to_draw.name_text.getTextureWidth() / 2 + player_to_draw.x - blocks->view_x, gfx::getWindowHeight() / 2 - PLAYER_HEIGHT / 2 + player_to_draw.y - blocks->view_y - player_to_draw.name_text.getTextureHeight() - HEADER_PADDING);
}
