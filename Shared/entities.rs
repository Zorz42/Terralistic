...
impl Entity {
    ...
    pub fn is_colliding(&self, blocks: &Blocks, direction: Direction, colliding_x: f32, colliding_y: f32) -> bool {
        self.is_colliding_with_block(blocks, direction, colliding_x, colliding_y)
    }
    pub fn is_colliding_with_block(&self, blocks: &Blocks, direction: Direction, colliding_x: f32, colliding_y: f32) -> bool {
        if colliding_x < 0.0 || colliding_y < 0.0 ||
            colliding_y >= blocks.get_height() as f32 * BLOCK_WIDTH * 2 - self.get_height() ||
            colliding_x >= blocks.get_width() as f32 * BLOCK_WIDTH * 2 - self.get_width() {
            return true
        }
        let starting_x = (colliding_x / (BLOCK_WIDTH * 2) as f32) as i32;
        let starting_y = (colliding_y / (BLOCK_WIDTH * 2) as f32) as i32;
        let ending_x = ((colliding_x + self.get_width() as f32 - 1) / (BLOCK_WIDTH * 2) as f32) as i32;
        let ending_y = ((colliding_y + self.get_height() as f32 - 1) / (BLOCK_WIDTH * 2) as f32) as i32;
    ...
    pub fn update_entity(&mut self, blocks: &Blocks) {
        ...
        let prev_y = self.y;
        let y_to_be = self.y + self.velocity_y / 100;
        let move_y = y_to_be - y;
        ...
        let prev_x = self.x;
        let x_to_be = self.x + self.velocity_x / 100;
        let move_x = x_to_be - x;
        let x_factor = if move_x > 0.0 { 1.0 } else { -1.0 };
        let has_collided_x = false;
    ...
    pub fn is_touching_ground(&self, blocks: &blocks) {
