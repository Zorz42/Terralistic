        ...
        methods.add_meta_method_mut(rlua::MetaMethod::NewIndex, |_lua_ctx, this, (key, value): (String, rlua::Value)| match value {
            rlua::Value::Integer(value) => {
                match key.as_str() {
                    ...
                    _ => {
                        return Err(rlua::Error::RuntimeError(format!("{key} is not a valid field of Wall for integer value")));
                    }
                };
                Ok(())
            }
            rlua::Value::String(value) => {
                match key.as_str() {
                    ...
                    _ => {
                        return Err(rlua::Error::RuntimeError(format!("{key} is not a valid field of Wall for string value")));
                    }
                };
                Ok(())
            }
            _ => Err(rlua::Error::RuntimeError("Unknown type for Wall".to_owned())),
        });
    }
}
