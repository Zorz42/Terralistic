...
impl PauseMenu {
    pub fn new(graphics: &gfx::GraphicsContext, settings: Rc<RefCell<Settings>>, global_settings: Rc<RefCell<GlobalSettings>>) -> Self {
        let mut resume_button = ui::Button::new(|| {});
        let mut settings_button = ui::Button::new(|| {});
        let mut quit_button = ui::Button::new(|| {});
        resume_button.texture = gfx::Texture::load_from_surface(&graphics.font.create_text_surface("Resume", None));
        resume_button.scale = 3.0;
        resume_button.pos.0 = -ui::SPACING + 10.0;
        resume_button.pos.1 = ui::SPACING;
        resume_button.orientation = ui::TOP_RIGHT;
        ...
        settings_button.pos.0 = -ui::SPACING + 10.0;
        settings_button.pos.1 = 2.0 * ui::SPACING + resume_button.get_size().1;
        settings_button.orientation = ui::TOP_RIGHT;

        quit_button.texture = gfx::Texture::load_from_surface(&graphics.font.create_text_surface("Quit", None));
        quit_button.scale = 3.0;
        quit_button.pos.0 = -ui::SPACING + 10.0;
        quit_button.pos.1 = 3.0 * ui::SPACING + settings_button.get_size().1 + resume_button.get_size().1;
        quit_button.orientation = ui::TOP_RIGHT;

        let mut back_rect = ui::RenderRect::new(gfx::FloatPos(-1000.0, 0.0), gfx::FloatSize(0.0, 0.0));
        back_rect.fill_color = ui::BLACK;
        ...
        let rect_width = f32::max(resume_button.get_size().0, f32::max(settings_button.get_size().0, quit_button.get_size().0)) + 2.0 * ui::SPACING;
    ...
    pub fn init(&mut self, graphics: &gfx::GraphicsContext) {
        self.settings_menu.init(graphics, &self.back_rect.get_container(graphics, &ui::Container::default(graphics)));
    }

    pub fn render(&mut self, graphics: &mut gfx::GraphicsContext) {
        self.in_settings &= self.open;
        let parent_container = self.back_rect.get_container(graphics, &ui::Container::default(graphics));
