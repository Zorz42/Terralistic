...
User requirements:
- Write code that is short and understandable
- Do not write long comments explaining everything unless necessary
- I want to have as few lines as possible in the repository
- Document everything in a very informative, objective and concise format

A Terraria-like 2D sandbox game in Rust. Single binary that runs as client, server, or
server-with-GUI. Rendering is wgpu through a hand-written UI toolkit, with winit for the window
...
concept or import `crate::shared`, `crate::client` or `crate::server` — true today and worth
checking before you add to one. Each library's `mod.rs` opens with what it does **and what it
deliberately does not do**; the second half is what stops game logic drifting in.
`libraries/README.md` is the index, `docs/LIBRARIES.md` the record of what was left in the game.
...
**The profiles are tuned for the loop you are actually in.** `dev` and `test` rebuild in ~2s
after a one line edit; `release` takes 10s because it optimises without linking the whole
program; `dist` takes a minute or more because it does. LTO on a profile you rebuild costs
that every time — it once made `cargo test` a 62 second wait to run a two second suite.

CI enforces `cargo fmt --check` and `cargo clippy --all-targets -- -D warnings`; the tree is
clippy clean, so keep it that way rather than dropping the flag.
...
~135 crate-level attributes, 108 of them `#![warn(clippy::...)]`. **That list is the project's
style contract**: `unwrap_used`, `expect_used`, `panic`, `indexing_slicing` and `todo` are all
warned on. Prefer `Result` + `anyhow` and `.get()` over indexing. When you must violate one,
add a local `#[allow(...)]` with a reason comment, which is the existing convention.
...
hash of `TypeId::of::<T>()` and `data` is postcard. Receivers call
`packet.try_deserialize::<SomeType>()`, which returns `None` if the hash doesn't match.

**All serialization goes through `libraries/serialization`** — packets, world saves and `.mod`
files alike. That module picks the format (postcard: little endian, LEB128 varint) in one
place. Don't call `postcard::` directly; when the backend changes, that file is meant to be
the only edit, and when bincode went unmaintained it very nearly was.

**There is no packet registry.** Any serializable struct is a packet, and packet structs live
next to their subsystem (`BlockChangePacket` in `shared/blocks/blocks.rs`, `InventorySwapPacket`
in `shared/inventory/`).

Handshake: the client's greeting is `VersionPacket` then `NamePacket`; the server checks both
and replies `WelcomeCompletePacket`, which ends the welcome phase. Welcome-phase packets are
...
- **The version check is the game's version, not the compiler's.** `TypeId` is not stable
  across rustc versions, so two builds of the same game version by different compilers still
  fail silently — every packet deserializes to `None`.
...
- Transport is `message-io` FramedTcp. The bind address is explicit per server via
  `BindAddress`: dedicated uses `AllInterfaces`, singleplayer `Loopback`. **Keep singleplayer
  on loopback** — it runs a real `Server`, and binding it wide puts every singleplayer world
...
- **The bind happens on the networking thread, so its failure arrives late.** `init` returns
  before the port is attempted, and the thread's `Result` only surfaces when
  `ServerNetworking::update` joins the finished thread. A server whose port is taken looks
  like one that started and accepts nobody, so the error names the address it could not bind.
...
- The server hands over **every** packet from every connected peer, including one it is about
  to refuse. `ServerNetworking::handle_packet` is the version and name check;
  `PacketServer::disconnect` is how it acts.
- The client's handshake is a greeting plus a predicate saying which packet ends it.
  `Received::during_handshake` is how the game knows which traffic was the welcome.
- `net::ClientError` is a *kind*, not a string. The transport knows the socket shut; only
  `ClientNetworking::explain` knows a close mid-handshake means the server refused this
  client. Player-facing wording lives there.

Both modules run a dedicated thread with an mpsc channel pair in and out; the main loop only
touches the channels, never the socket. **The client's welcome phase runs a 1ms signal timer of
its own**, and that is what makes it cancellable — everything else in that phase reacts to the
server, so with no timer nothing ever looked at `is_running` and `stop()` waited forever. The
timer is armed once before `listener.for_each` and re-armed by whichever branch handles it, so
there is exactly one chain: don't send a second signal when the welcome completes.

### Mod system (Lua via rlua)

`libraries/scripting`. Each `ScriptModule` owns its own `Lua` state, its minified source, and a
`HashMap<String, Vec<u8>>` of resources keyed with `:` as separator (`blocks:dirt.opa`). A
`ScriptHost` drives a set of them.

The on-disk `.mod` format is `snap(postcard(ScriptModuleData))` — see
`libraries/scripting/module_data.rs`, a deliberately dependency-free leaf so the build script
can write mods without pulling in Lua. `build_main.rs` names that leaf file rather than the
`scripting` module, which is what keeps rlua out of `[build-dependencies]`.

A handle type is made passable to lua by `script_handle!(Type)` (or `script_handle!(Type, eq)`
where lua should be able to compare two), a macro because the orphan rule stops either side
writing the blanket impl. Anything that is not a handle is a lua conversion error.

Rust exposes functions to Lua with the `terralistic_` prefix — `ScriptHost::add_global_function`
adds it, so `add_global_function("get_block", ..)` is `terralistic_get_block(x, y)` from Lua.
The prefix is `shared::MOD_FUNCTION_PREFIX`, handed to the host at construction: it keeps *this*
game's names out of a mod's way, so it lives with the game rather than the library. The
registration sites are the `mod_interface.rs` files:
...
`tools.lua`, `recipes.lua`, `biomes.lua`, `commands.lua`. The build script concatenates **all**
`.lua` files into one chunk, so they share one global namespace and load order is filesystem
order — which is why `main.lua`'s `init()` calls the `register_*` functions explicitly in
dependency order (tools → blocks → walls → items → recipes).
...
`update_block` on every cell pushed a `BlockUpdateEvent` each — 5.4M for the default world —
which sat queued until the first `update()`, then went to every subsystem and to lua's
`on_block_update` one at a time: 18 seconds and about a gigabyte, *after* the loading screen
closed, so a newly generated world looked exactly like a hang. The scan for where to start
still reads every cell, so it takes the lock once and clones nothing.
...
and a save can be written before the first update, so a canopy half grown then is half grown
on disk.

**Sea level is a percentile of the terrain, not a constant.** `FLOODED_COLUMN_FRACTION` is
0.15, so sea level is the height 15% of columns are lower than: the lowest ground of whatever
world the generator is handed floods, and a perfectly flat one does not. A fixed y would be
underground or in the sky for any mod that picked different terrain heights, which are lua.

**The fill walks down from the top of the world, not from sea level**, stopping at the first
non-ghost block. That is the difference between lakes and a hillside with water hidden inside
it: from sea level, any cave crossing that line fills whether or not anything could pour into
it — a third of the columns of a test world instead of the intended 15%. From the sky, a column
whose ground is above sea level stops at its own surface, while a pit or a cave mouth open to
the sky floods, which is what a hole in a shoreline should do.
...
the authoritative copy, `client/game/liquids.rs` draws what it is told.
...
Physics is in `shared/entities/entities.rs`: `liquid_submersion` samples the cell the entity's
middle is in, and the level scales both the extra drag (the type's `speed_multiplier`) and the
buoyancy that cancels most of that tick's gravity. Holding jump under the surface swims
(`update_players_ms`). Both run on client and server off each side's own copy, which is what
keeps prediction agreeing with the authority.
...
`set_liquid`, which is **server only** like `set_block` because the client's grid is a replica.
`/water <x> <y> [level]` pours some in by hand; there is no bucket item.

**Liquids draw in front of the player**, after the entities and before the floating damage text
(`core_client.rs`), so a player wading through translucent water is seen through its surface.
...
widget toolkit — layout, input routing, widgets, menus. Neither has game knowledge; treat both
as vendored.

It is a module split inside one crate, so the two do refer to each other. One edge is not going
away without a bigger change: `render_inner` and `update_inner` take a `&mut GraphicsContext`
because a few widgets genuinely build textures, and `RenderRect` reaches
`GraphicsContext::blur_rect` and `shadow_context`, which are `pub(crate)` for it.
...
**Every fade and slide in the game is `ui::approach(value, target, smooth_factor, epsilon)`**
per ready `timing::FixedStep::for_animation` frame; don't hand-roll another one. The epsilon is
not decoration — it is what makes an animation *land* on its target instead of nearing it
forever, which a hand-rolled `value -= value / factor` does not.
...
never hovered. Clicking is `gfx::ClickTracker`, which `Button` and `Toggle` each own one of —
see the gotcha below for the rule it encodes.
...
`update_inner` takes a `&mut GraphicsContext` because a few menus genuinely build textures in
one, which leaves any animation stepped there unreachable from `cargo test`. Where the stepping
needs nothing from the context, split it into a `pub(super)` method and call that —
`Scrollable::advance_frame` and `TextInput`'s three geometry methods are the pattern.

`UiContext` (`ui/ui_context.rs`) is the whole non-rendering surface of `GraphicsContext`:
window size, mouse position, key states, clipboard. `GraphicsContext` implements it, and so
does `HeadlessContext`, a `#[cfg(test)]` struct whose inputs are plain fields — which is what
makes hit testing, layout and event routing testable in CI with no window. `libraries/ui/tests.rs`
is that suite.

**Keep new widgets off `GraphicsContext` in those two methods.** If an event handler needs to
draw or upload a texture, the work almost always belongs in `render_inner`. Three menus
genuinely do not fit — `world_creation` and `multiplayer_selector` build the next menu,
`settings_menu` applies vsync/scale — and they use the escape hatch
`UiContext::as_graphics_context() -> Option<&mut GraphicsContext>`, which is `None` headlessly.
Those branches are the only UI logic tests cannot reach. Companion `#[cfg(test)]` constructors
exist for the same reason: `Font::new_headless`, `Texture::new_sized`, `TextInput::new_headless`.

Rendering goes to an offscreen texture blitted to the window in `update_window()`, which is
what makes the blur and shadow effects possible.

**That offscreen is the display's real pixel size, not the logical one.** Layout is in logical
pixels either way — the transform onto clip space is a ratio and does not care how many pixels
the target has — but the real resolution lets a smooth animation move one device pixel at a
time instead of jumping two, and makes the final blit a copy rather than an upscale. It costs
~1 ms of GPU per frame (3.1 → 4.2 ms on an M-series Mac at 3340x2100). The size is chosen in
one place, `GraphicsContext::handle_window_resize`, and would make a reasonable setting.
...
winit's `run_app` callback was not worth it. It costs a repaint mid-resize-drag on macOS, and
`pump_app_events` is desktop only — which is all this builds for.
...
- **The pump happens at the end of `update_window`, not at the top of the frame.** On macOS
  ...
  stay outside the budget window. `get_event` only pumps before the very first frame
  (`window_pumped`).

The window also asks for focus on creation — a pumped loop leaves macOS not treating the app as
active, so it would open behind the terminal — and `key_states` is cleared on focus loss, so a
held key does not stick across an alt-tab.

The game draws at winit's true `HiDPI` drawable size rather than being smoothed up to it by the
compositor, which means the surface can exceed `Limits::downlevel_defaults`' texture cap. Hence
`downlevel_defaults().using_resolution(adapter.limits())`: with the plain defaults a 1670x1050
window on a 2x display fails `Surface::configure` outright.
...
- **`handle_window_resize` also owns the normalization transform**, and has to. The frame
  about to be recorded draws through whatever transform is in force *now*, so one set only at
  the end of `update_window` is the identity for the first frame — the whole window collapsing
  into the top-left two-by-two of clip space — and one size stale after every resize. Both
  last exactly one frame, which is why neither was noticed; removing the call brings them back.
...
- **The clear gets a render pass of its own**, on the frame texture, before any of that. Only
  the golden harness asks for one — the game draws an opaque background over the whole window —
  and `blur_over_a_cleared_frame` pins it: folded into a frame that opens with a blur, the
  clear lands on the *back* offscreen and the blur reads the previous frame straight back in.

##### The blur is the frame's one area-priced effect, so it runs small

Every pass is thirteen taps per pixel of the region and there are four to six of them, so a
menu blurring most of a fullscreen `HiDPI` window is seven million pixels through half a
billion samples — **7.3 ms of GPU in one command** at 3340x2100, which dropped a fullscreen
window to 45 fps. So `plan_blur` shrinks the region into a scratch texture, blurs it there and
stretches it back. The tap spacings are unchanged, so it is the same blur sampled on a coarser
grid, and a blur has nothing finer than its own kernel to lose: no channel moved more than 6 of
255 against the full resolution result.
...
- **The scratch pair is allocated at `MIN_BLUR_DOWNSCALE`**, so a whole-window region fits at
  the finest downscale and `plan_blur` never has to fit one that does not. A smaller region
  uses the top left corner of the pair and clamps its sampling to it.
- **The blur samples linearly, and it is the only thing in the toolkit that does.** Stretching
  back with `NEAREST` is visible blocks. It gets its own bind group layout and sampler rather
  than relaxing the shared one, which declares itself non-filterable precisely so a pixel art
  game cannot smooth anything by accident.
- **The upsample is an ordinary `Segment::Draw`**, not a pass of its own, so it joins the draw
  pass after the blur. On a tiled GPU a pass costs its whole attachment however small the quad
  is, and the attachment here is the frame.

`execute` plans before it encodes: `Plan` collects a `Uniforms` per draw plus a flat list of
`Segment`s, because wgpu wants all the uniform data written before any is encoded.

##### The device is global, and deferred release is load-bearing

`Texture::load_from_surface(&surface)` is called from ~80 places and takes no context. The
choice was to thread a `&Device` through all 80 or keep the coupling and write it down;
`gpu_device.rs` is the latter, a `OnceLock<GpuDevice>` the renderer publishes once. One
consequence is an improvement: **creating a texture with no device is not an error**, it
produces one that knows its size and owns nothing, so layout works headlessly and `cargo test`
can build real textures.

Resources live in a registry there and commands name them by id, so a resource can be dropped
while a command still refers to it — not a corner case: menus build a text texture inside
`render_inner` and drop it there, every world chunk replaces its whole `RectArray` when it
changes, and the golden cases draw from temporaries. `Drop` parks the id and `execute` sweeps
after the frame is submitted. **Don't make that an immediate removal** — the command would
resolve to nothing and the draw would silently vanish. Mutating a live mesh mid-frame would
still be wrong, but nothing does it: every `RectArray` mutation replaces the whole object.
...
`RectArray` in RAM per chunk — ~50 KB each across three caches of `MAX_LOADED_CHUNKS`. An
`upload` with nothing staged therefore leaves an uploaded mesh alone rather than emptying it.
...
The window is hidden, so running them does not flash windows across the desktop. The harness is
also the one caller that draws at the window's *logical* resolution
(`render_at_logical_resolution`), and captures from the offscreen texture rather than the
surface — between them, a golden is the same image whatever the machine's DPI.

These are the second of **two** tiers. The draw-list tests in `graphics/tests.rs` assert on the
commands a primitive records and run under `cargo test`; these assert the backend turns those
commands into the right pixels. **Every primitive reaches the first tier**, because building
one without a device is a supported state — so "does the shadow cover the whole border" is an
ordinary `#[test]` and only "is it the right colour" needs a GPU. They are also the real test
...
cannot be compared against a different case's image. Adding one is a line: `case_foo`, or
`case_foo: BLURRY` where `EXACT` will not do.

Determinism is the whole game, and the toolkit fights it in three places, each with a
...
and scale fades (`GraphicsContext::settle_animations`), and hover states reading the real
...
`render_rect_blur` is `BLURRY`, the gaussian shader's float error differing between drivers.
The two knobs compose rather than alternate: a pixel is over tolerance when a channel moved
further than `max_channel_delta`, and the case fails when more than `max_differing_fraction`
of the frame is. As an `||` the channel limit was unreachable. The shadow looks like it belongs
in that group and does not — `ShadowContext` bakes its gaussian into a CPU `Surface` and draws
it as a `NEAREST` texture. Keep new cases `EXACT` unless a shader is genuinely involved; a
loose tolerance once absorbed a real one-pixel shift in the text input cases.

One known-bad behaviour is recorded as-is rather than fixed: **alpha eaten by blending.** The
blend factors apply to alpha as well as colour, so anything translucent lowers the
framebuffer's alpha below 1. Invisible on screen, since the blit ignores alpha, but a capture
records it.
...
`WgpuBackend::plan_command` rounds a texture draw's destination to a whole pixel, and that is
not cosmetic. Sampling is `NEAREST`: a destination pixel takes whichever texel its centre falls
in. On a whole pixel with an integer scale those centres land halfway through a texel and every
texel gets the same number of pixels. Half a pixel off they land *on* the boundaries, where the
side comes down to the last bit of an interpolated float — so a 3x glyph pixel came out 2 or 4
wide, unevenly along the string, and at scale 1 the one pixel gaps between strokes vanished.

Layout puts things on half pixels constantly — anything centred in an odd-width parent, in a
`Scrollable`, or mid-animation — so this hit every row of the world list.
`text_on_fractional_offsets` is the case that catches a regression: the same string at four
sub-pixel offsets, which must come out identical.

The grid is the **offscreen's**, the display's real resolution, so on `HiDPI` this still leaves
half a logical pixel for an animation to move along, and nothing is lost — nothing filters, so
there is no sub-pixel detail to lose. Meshes are **not** snapped: `RectArray` maps texture
coordinates per vertex, and the world would jitter against the camera.
...
own falls back to breaking at whichever character overruns, which keeps a narrow limit from
wrapping forever. The space a wrap steps over is neither drawn nor counted — it would indent
the line it lands on and push the block past the limit it just wrapped to. Both apply *only*
when there is a limit, so measuring a prefix (which `TextInput` always does without one) stays
additive. The only text in the game that wraps is `ChoiceMenu`'s title, where errors are shown.

**The space is the only character whose advance is not its glyph's width**, and only in a
proportional font, where trimming leaves its glyph empty. A mono font's space was already
padded to the common width by `load_surfaces`, and adding `SPACE_WIDTH` on top made it the one
character breaking the grid — in a font the server console draws a column of timestamps in.

`TextInput`'s view into a value too long for the box centres on the cursor while selected and
shows the tail when not. Nothing here clips, so anything let off the left edge goes on painting
a white bar over the widget beside it — which is why `cursor_rect` clips itself to the widget,
a *selection* being as wide as the text it covers. Centring rather than pulling the view just
far enough is what makes both directions behave: with the moving end held against the left
edge, everything shift-and-right-arrow selected scrolled off behind it. The geometry is
`view_offset` / `visible_text_rect` / `cursor_rect`, kept out of `render_inner` so that a
`Font::new_headless` is all a test needs.

Six UI pieces predate the `UiElement` trait and are hand-rolled, marked by `//TODO` comments in
`client/game/{chat,debug_menu,inventory,pause_menu,respawn_screen}.rs` and
`client/menus/settings_menu.rs`. Converting one is a good self-contained task.
...
**Every clock in the game is a `libraries/timing` type**, with one accumulator behind all of
them. There were five hand-rolled ones, two already fixed for the same class of bug months
...
- **The two `FixedStep` constructors are the catch-up policy, and picking the wrong one is a
  bug.** `new` owes every step, because a simulation that skips one has silently run slower and
  nothing downstream can tell. `for_animation` caps the backlog at `MAX_CATCHUP_FRAMES`,
  because frames nobody saw are worth nothing — the pause menu's buttons, built when the world
  loads and first drawn an hour later, owed 3.6 million steps on one frame. Every widget that
  animates owns a `for_animation` one.
- `Interval` is the third rule: things sharing one simulated clock, each at its own rate, and
  **a missed one is not owed** — the opposite of `FixedStep::new`, and what each liquid type's
  `flow_time` runs on.
- `DeltaTimer::tick` answers `None` on its first call, the time since construction being however
  long starting up took rather than a frame anybody rendered. That is the server's "skip this
  update".
...
That is the whole mechanism keeping the frame rate up while a world loads. Anything slow
*before* `walls.render` starves it, and the failure mode is not a crash but a world that draws
its blocks immediately and takes minutes to finish its walls and lighting. Two things have done
this already, both from the winit port: querying the window size per call, and pumping the
event loop at the top of the frame instead of at the end of `update_window`. Measure
...
from one that is slow, and the client would wait forever on a port whose server has died.
Multiplayer passes `None` — a remote server's health is not this process's to know. Either way
a failed join ends on a `ChoiceMenu` naming the error.

**The join draws its own frames, because nothing else is drawing.** `run_game` is called from
inside the menu loop's `open_menu`, so that loop is stopped for the whole join — the handshake,
the welcome packets, mod init, the light grid, resources. `JoinScreen` in `core_client.rs` is a
`MenuBack` plus the menus' own `LoadingScreen`, one frame per phase with the phase named.
Anything long added to the join belongs behind a `frame` call with a name on it, and the wait
for the handshake is a frame per iteration rather than a sleep — `update_window` sleeps out the
rest of the frame's share of the clock anyway.

### Persistence

A world save is a fixed header followed by `postcard(HashMap<String, Vec<u8>>)` with keys
`blocks`, `walls`, `liquids`, `players`; those three grids are additionally snap-compressed.
Written to `server_data/server.world` relative to the process CWD. Client settings are JSON at
`<data_dir>/Terralistic/settings.txt`.

The header is `b"TERRAWLD"` then `WORLD_SAVE_VERSION` as a little endian `u32`, deliberately
**outside** anything a serializer wrote. Versions 1 and 2 kept it *inside* the encoded map,
which works right until the encoding is the thing that changed — and then the version cannot be
read either, so the player gets a decode error instead of being told their world is old. The
shape is `WORLD_SAVE_FORMAT` in `shared/versions.rs`, a `container_file::ContainerFormat`, and
its `read_header` runs before the body is touched.

The liquid grid is two bytes a cell and is written whether or not there is any liquid in it —
about 0.5 MB of a 1.6 MB save. That is the price of a dense `Vec` grid, right for a world with
an ocean and paid by a dry one too.
...
**not** committed — `.gitignore` carries `Build/` and `**/*.mod`, so every checkout builds its
own. The only `.opa` files in git are the 46 golden images, which are `snap(postcard(Surface))`
too and so have to be converted if the serialization format changes.

The `.mod` build is reproducible: identical sources produce identical bytes. Resources were
once serialized from a `HashMap`, whose iteration order Rust randomises per process, so the
committed artifact changed on every build. `GameModData.resources` is a `BTreeMap` to keep that
stable — don't change it back.
...
- **Wait for the listener, by asking it.** Both networking layers bind on a background
  thread, so `wait_until_listening` is what makes an immediate connection reliable. It takes a
  predicate backed by `ServerNetworking::is_listening`, an atomic the binding thread sets, and
  **must not go back to probing the port**: "has the port stopped being bindable" has to *bind*
  it to find out, which is the very collision it is watching for. Polling every millisecond it
  regularly won the race, the server's `listen` then failed with `AddrInUse`, and since that
  surfaces only through a later `update()` the probe waited out the full 30 s timeout — a
  1-in-10 flake landing on whichever test lost the toss. `SO_REUSEADDR` does not save you:
  it waives `TIME_WAIT`, not a live listener.
...
- **An id is a handle, not a position** — both `libraries/registry` and `Settings`. Numbers
  come from a counter that never reuses one, so the lights toggle, registered when a world
  loads and removed when it closes, has a higher id on every world opened. `settings_menu`
  used to place each row at `id * row height` and watched it drift down the screen; it lays
  out by position in the sorted list instead. `RegistryId::index` is an implementation detail
  of the lookup and returns `Option`, so the "undefined" value every one of these newtypes has
  resolves to nothing rather than to whatever a negative index casts to.
- **`build_main.rs` declares its own narrow module tree.** It names individual leaf files
  ...
  the game's dependency tree — naming `scripting` would pull in rlua. Module *paths* must still
  match `main.rs`, since those files refer to themselves as `crate::libraries::…`. If the build
  script needs another type, move that type to a dependency-free leaf rather than widening the
  declaration.
...
- `core_server.rs` keeps the tick counters as `Server` fields, so two servers in one process
  do not share them — the integration tests rely on that. The channel back to the ui is still
  process-global (`UI_EVENT_SENDER`), because `print_to_console` is a free function with no
  `Server` to reach through: the first non-`None` sender wins.
- **The singleplayer loading screen closes on an empty status string and nothing else**, so a
  private server thread ending without emptying it strands the player for good. That is a `Drop`
  guard in `private_world.rs` rather than an `if result.is_err()`, because a panic in the server
...
- **`Server::run` shuts the networking thread down on every exit path**, which is why it wraps
  ...
  receiving end dropped, so it accepted clients it could tell nobody about and they waited for
  a welcome forever — and the port stayed taken, so every world opened afterwards hung the same
  way. The world is deliberately *not* saved on that path: the error may be about the world.
...
- Test files follow one convention: a `tests.rs` beside the module it covers, declared in the
  neighbouring `mod.rs`, containing `#![cfg(test)] mod tests { .. }`. Every module directory
  has one; tests that span subsystems live in `integration_tests/` instead.
- Tests never need a graphics context. UI tests drive real widgets through
  `ui::HeadlessContext`, so the whole suite runs on CI's headless Ubuntu.
...
- **A click is a press and a release on the same widget** — `ui::ClickTracker`, which `Button`
  and `Toggle` both hold. Half a click does nothing in either direction. The remembered press
  is deliberately *not* cleared by the release that consumes it, because several menus hold
  their buttons as sub-elements **and** dispatch to them again from `on_event_inner`, so one
  release reaches a button twice and the menu reads the second answer — worth knowing before
  adding a button with a side effect in its closure. A menu that dispatches to buttons itself
  has to forward *every* event, not only the release; `choice_menu` did the latter and its
  buttons went dead the moment the press started mattering. A widget that reacts to a bare
  release is the bug, not the pattern: `settings_menu` read `Toggle::hovered` and so flipped a
  setting for any release landing on a toggle. It reads `toggle.toggled` back instead — the
  toggle is a sub-element, so it has already answered the same event.
- **`gfx::Texture::load_from_bytes` turns a bad asset into an empty texture, not an error.** A
  corrupt `.opa` should leave a hole in the screen rather than take the process down, and it is
  one fallback in one place — the ten call sites that spelled it out invented three sizes.
- **A `gfx::Surface` is checked against its own size when it is deserialized**, because nothing
  downstream re-checks: `GpuDevice::create_texture` tells wgpu the texture is `get_size()` big
  and hands it `pixels`, and a mismatch is a validation error, which is a panic. Surfaces come
...
- Two `create` methods differ: `Blocks::create` fills the grid with air, `Walls::create` with
  `WallId::undefined()`, so reading a wall before setting one errors. Only `create_from_wall_ids`
  calls it and that overwrites everything, so the game never hits it — but calling
  `Walls::create` directly is a trap. The fill is an argument to `Grid::filled` at both call
  sites, so it is at least visible rather than implied.
